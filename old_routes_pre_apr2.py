...
async def execute_action(request: ExecuteActionRequest):
    ...
    try:
        ...
        # Note: We skip saving to Redis conversation_history ΓÇö Peppi already
        ...
        for attempt in range(1, MAX_EMPTY_RETRIES + 1):
            ...
            try:
                ...
                if payloads and len(payloads) > 0:
                    ...
                    # Gemini/OpenClaw may return in different formats ΓÇö try all known fields
                    clean_response = (
            ...
            if has_side_effects:
                # Action already executed ΓÇö don't retry, use fallback
                clean_response = "Done! Your request has been processed."
                logger.warning(
                    f"[{session_id}] Empty payloads but action '{detected_action}' "
                    f"likely already executed ΓÇö using fallback response"
                )
                break

            # Chat-like action ΓÇö safe to retry
            if attempt < MAX_EMPTY_RETRIES:
                ...
                logger.error(
                    f"[{session_id}] Empty payloads after {MAX_EMPTY_RETRIES} attempts ΓÇö returning error"
                )
        ...
        if not tokens_used:
            ...
            logger.info(f"Token estimation: input={input_chars}c output={output_chars}c ΓåÆ ~{tokens_used} tokens")

        # Skip saving assistant response to Redis ΓÇö Peppi manages its own history
